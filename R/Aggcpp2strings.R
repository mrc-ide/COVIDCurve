...
make_user_Agg_logprior <- function(IFRmodel, reparamIFR, reparamInfxn, reparamKnots, reparamSpec) {
  ...
  makeSerotestpriors <- c(
    paste0("R::dbeta(sero_rate,", Serotestparams$dsc1[Serotestparams$name == "sero_rate"], ",", Serotestparams$dsc2[Serotestparams$name == "sero_rate"], ", true) +"),
...
make_user_Agg_loglike <- function(IFRmodel, reparamIFR, reparamInfxn, reparamKnots, reparamSpec) {
  ...
  #......................
  if (reparamSpec) {
    specreparam <- paste0(maxMa, " = 1/spec *", maxMa, ";")
  } else {
    specreparam <- ""
  }
  ...
  ret <- c("SEXP loglike(Rcpp::NumericVector params, int param_i, Rcpp::List data, Rcpp::List misc) {",
           ...
           node_xvec,
           node_yvec,
           specreparam, # must be before mavec to account for potential reparam/scalar of Ma relative to Spec
           mavec,
           loglike,
